...
namespace RalseiMod.Skills.SkillDefTypes
{
    class DualSkillDef : SkillDef
	{
		...
		public override EntityState InstantiateNextState([NotNull] GenericSkill skillSlot)
		{
			...
			{
				SerializableEntityStateType sest = this.activationState;
				entityState = EntityStateCatalog.InstantiateState(ref sest);
			}
            else
			{
				SerializableEntityStateType sest = this.alternateActivationState;
				entityState = EntityStateCatalog.InstantiateState(ref sest);
			}
